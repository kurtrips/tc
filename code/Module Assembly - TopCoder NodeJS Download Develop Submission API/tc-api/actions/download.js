...
var downloadDevSubmission = function (api, connection, dbConnectionMap, next) {
    ...
    var helper = api.helper,
        ...
        uploadId = Number(connection.params.uploadId),
        noRights = true,
        basicInfo,
        myResourceRoles,
        myResourceIds,
    ...
    async.waterfall([
        function (cb) {
            ...
            var error = helper.checkPositiveInteger(uploadId, 'uploadId') ||
                helper.checkMaxInt(uploadId, 'uploadId');
            if (error) {
                cb(error);
                return;
            }
            ...
            if (!basicInfo[0].is_submission) {
                cb(new BadRequestError('This upload is not a submission.'));
                return;
            }
            ...
            if (noRights && basicInfo[0].is_upload_deleted && _.intersection(myResourceRoles, viewAllSubmissionRoles).length === 0) {
                cb(new NotFoundError('This upload has been deleted.'));
                return;
            }
