    ...
    public class SoapMessageReferenceLoader : IReferenceLoader
    {
        ...
        public byte[] LoadReferenceData(string uriString)
        {
            ...
            try
            {
                ...
                using (Stream stream = soapMessage.Stream)
                {
                    stream.Position = 0;
                ...
                if (nodeFound == null)
                {
                    throw new ReferenceLoadingException("No element with id " + uriString + " was found in soap message");
                }

                return Encoding.Unicode.GetBytes(nodeFound.OuterXml);
            }
        ...
        private void FindNodeWithId(XmlNode element, string id, ref XmlNode foundNode)
        {
            ...
            XmlNode anIdNode = element.Attributes.GetNamedItem("Id");
            if (anIdNode != null)
            {
                if (anIdNode.InnerXml == id)
                {
                    foundNode = element;
                }
            }
