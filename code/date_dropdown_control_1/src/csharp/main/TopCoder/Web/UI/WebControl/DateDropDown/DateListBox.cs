...
{
    /// <summary>
    /// <para>This is a custom web-control that allows the user to select a single or multiple date/time items from a list.
    /// The class is inherited from the standard ListBox web-control and provides a special support for list items,
    ...
    /// The initial (single/multiple) selection of the control items can be configured through the implemented
    /// properties, or set explicitly.  The selected items can be easily retrieved (as DateTime or DateTime[] object) at
    /// any time by using a provided function. Note, this control fully maintains all formatting and rendering
    /// capabilities of the underlying ListBox control. </para>
    /// </summary>
    ///
    /// <threadsafety>This class is not thread-safe. It inherits from the not thread-safe parent class (ListBox),
    /// and there are no needs to make web-controls thread-safe.</threadsafety>
    ///
    ...
    public class DateListBox : ListBox
    {
        /// <summary>
        ...
        public virtual DateTime SelectedDate
        {
            get
            {
                DateTime[] selectedDates = SelectedDates;
                if (selectedDates.Length > 0)
                {
                    //Return the first selected item
                    return selectedDates[0];
                }
                throw new DateDropDownException("No dates are selected in the DateListBox.");
            }
        }

        /// <summary>
        /// <para>Represents the getter property for the selected items of this control internal list.</para>
        /// </summary>
        /// <value>An array containing all the selected dates in the listBox.</value>
        public virtual DateTime[] SelectedDates
        {
            get
            {
                ...
                foreach (ListItem item in Items)
                {
                    if (item.Selected)
                    {
                        ret.Add(DateTime.ParseExact(item.Text, DisplayDateFormat, CultureInfo.InvariantCulture));
                    }
                }

                return (DateTime[]) ret.ToArray(typeof(DateTime));
            }
        }
        ...
        public int RemoveDateItem(DateTime dateItem)
        {
            ...
            {
                throw new DateDropDownException("Could not convert date to the desired format.", e);
            }
